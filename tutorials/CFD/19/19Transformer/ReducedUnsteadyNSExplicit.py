...
class ReducedUnsteadyNSExplicit:
    ...
    def __init__(self, fluxMethod):
        ...
        self.N_BC = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/N_BC.npy")
        self.Nphi_u = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/Nphi_u.npy")
        self.Nphi_p = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/Nphi_p.npy")
    ...
    def _solve_consistent(self, vel):
        ...
        C_tensor = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/C_" + str((0)) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + "_t.npy")        

        Cf_tensor = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/Cf_" + str(0) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + "_t.npy")        

        Ci_tensor = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/Ci_" + str(0) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + "_t.npy")        

        RD_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/RD/RD" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        RC_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/RC/RC" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        SD_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/SD/SD" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        SC_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/SC/SC" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        W_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/W_" + str(NUmodes) + ".npy")

        BP_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/BP" +  "_" + str(NPmodes) + ".npy")

        P_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/P" + "_" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + "_" + str(NPmodes) + ".npy")

        B_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/B" + "_" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        K_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/K" + "_" + str(i) + "_" + str(NUmodes) + "_" + str(NSUPmodes) + "_" + str(NPmodes) + ".npy")

        I_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/I" + "_" + str(NUmodes) + ".npy")

        KF_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/KF" + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        DF_matrix = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/DF" + "_" + str(NUmodes) + "_" + str(NSUPmodes) + ".npy")

        cTotalTensor = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/ITHACAoutput/Matrices/cTotalTensor.npy")

        # Create and resize the solution vectors

        a_o = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/a_o_con" + ".npy")
        a_n = np.zeros(a_o.size)
        b = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/b_con" + ".npy")
        c_o = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/c_o_con" + ".npy")
        x = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/x_con" + ".npy")
        presidual = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/presidual_con" + ".npy")
        RHS = np.load("/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/RHS_con" + ".npy")
        ...
        # Modello LSTM
        # lstm_model = tf.keras.models.load_model('./MLP/trained_model.keras')
        # tr_model = tf.keras.models.load_model('./Transformer/transformer_model.keras')
        # x_scaler = joblib.load("./Transformer/x_scaler.pkl")
        # y_scaler = joblib.load("./Transformer/y_scaler.pkl")
        ...
        for out_iterator in range(len(self.online_solution)):
            ...
            for i in range(int(self.N_BC) + 1):
                ...
                filename_Div = f"/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/LinSysDiv_{i}.npy"
                LinSysDiv.append(np.load(filename_Div))
                
                filename_Conv = f"/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/LinSysConv_{i}.npy"
                LinSysConv.append(np.load(filename_Conv))
 
                filename_Diff = f"/home/nrooho/ITHACA-FV/tutorials/CFD/19/19Transformer/file_python/LinSysDiff_{i}.npy"
                LinSysDiff.append(np.load(filename_Diff))
            ...
            # Caricamento modello e scaler una volta sola
            if not hasattr(self, "tr_model"):
                # from tensorflow.keras.models import load_model
                # import joblib
                self.tr_model = tf.keras.models.load_model("Transformer/transformer_model.keras")
                self.x_scaler = joblib.load("Transformer/x_scaler.pkl")
                self.y_scaler = joblib.load("Transformer/y_scaler.pkl")

            # Inizializza buffer temporale se non esiste
            if not hasattr(self, "input_history"):
                self.input_history = []

            # Concatenazione e scaling
            input_concat = np.concatenate((a_n.flatten(), b.flatten()), axis=0).reshape(1, -1)
            input_scaled = self.x_scaler.transform(input_concat)  # shape: (1, 20)

            # Aggiungi al buffer
            self.input_history.append(input_scaled[0])  # shape (20,)
            if len(self.input_history) > 15:
                self.input_history.pop(0)

            # Predizione solo se abbiamo abbastanza dati
            if len(self.input_history) == 15:
                transformer_input = np.array(self.input_history).reshape(1, 15, -1)  # shape: (1, 15, 20)
                nut_pred_scaled = self.tr_model.predict(transformer_input, verbose=0)[0]
                nut_coeffs = self.y_scaler.inverse_transform(nut_pred_scaled.reshape(1, -1))[0]
            else:
                nut_coeffs = np.zeros(self.y_scaler.mean_.shape)
    ...
    def reconstruct(self, Tr_Rec):

        os.makedirs(Tr_Rec, exist_ok=True)
        ...
        u_field_Tr = u_POD_matrix @ CoeffU_mat
        p_field_Tr = p_POD_matrix @ CoeffP_mat
        nut_field_Tr = nut_POD_matrix @ CoeffNut_mat

        print("u_field_LSTM", u_field_Tr.shape)
        print("p_field_LSTM", p_field_Tr.shape)
        print("nut_field_LSTM", nut_field_Tr.shape)

        np.save(os.path.join(Tr_Rec, "u_field_Tr.npy"), u_field_Tr)
        np.save(os.path.join(Tr_Rec, "p_field_Tr.npy"), p_field_Tr)
        np.save(os.path.join(Tr_Rec, "nut_field_Tr.npy"), nut_field_Tr)
